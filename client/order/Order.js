...
const Order = ({ classes, match }) => {
  ...
  const getTotal = () => {
    return state.order.products.reduce((a, b) => {
  ...
  return (
    <Card className={classes.card}>
      ...
      <Typography type="subheading" component="h2" className={classes.subheading}>
          Order Code: <strong>{state.order._id}</strong> <br/> Placed on {(new Date(state.order.created)).toDateString()}
      </Typography><br/>
      <Grid container spacing={8}>
          <Grid item xs={7} sm={7}>
              <Card className={classes.innerCardItems}>
                {state.order.products.map((item, i) => {
                  return  <span key={i}>
          ...
          <Grid item xs={5} sm={5}>
            <Card className={classes.innerCard}>
              ...
              <Typography type="subheading" component="h3" className={classes.info} color="primary"><strong>{state.order.customer_name}</strong></Typography><br/>
              <Typography type="subheading" component="h3" className={classes.info} color="primary">{state.order.customer_email}</Typography><br/>
              <br/>
              <Divider/>
              <br/>
              <Typography type="subheading" component="h3" className={classes.itemShop} color="primary">{state.order.delivery_address.street}</Typography>
              <Typography type="subheading" component="h3" className={classes.itemShop} color="primary">{state.order.delivery_address.city}, {state.order.delivery_address.state} {state.order.delivery_address.zipcode}</Typography>
              <Typography type="subheading" component="h3" className={classes.itemShop} color="primary">{state.order.delivery_address.country}</Typography><br/>
